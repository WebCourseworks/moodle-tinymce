...
//  USER is a persistent variable using sessions
    ...
    function html_header($course, $wdir, $formfield=""){
        ...
        if (!empty($_SERVER['HTTPS']) and $_SERVER['HTTPS'] != 'off') {
            $url = preg_replace('|https?://[^/]+|', '', $CFG->httpswwwroot).'/lib/editor/htmlarea/';
        } else {
            $url = $CFG->httpswwwroot.'/lib/editor/htmlarea/';
        }
                    ...
                    for(field in params) {
                        ...
                        switch(field) {
                            case "url"   :   upper.document.getElementById('f_url').value = value;
                                     upper.ipreview.location.replace('../htmlarea/popups/preview.php?id='+ <?php print($course->id);?> +'&imageurl='+ value);
                                break;
...
//  $wdir == / or /a or /a/b/c/d  etc
    ...
    if ($wdir == "/") {
        ...
        print "<img src=\"$CFG->httpswwwroot/lib/editor/htmlarea/images/folderup.gif\" height=\"14\" width=\"24\" border=\"0\" alt=\"".get_string('parentfolder')."\" />";
        print "</a></td>\n</tr>\n";
    }
